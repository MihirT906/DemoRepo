...
class Figma_Integration {
  ...
  async login() {
    ...
    let ret = await this.type("#email", String(this.username));
    if (ret.status == 102) {
      console.log("[-] " + ret.status + " : " + ret.message);
      return ret;
    }

    ret = await this.type("#current-password", String(this.password));
    if (ret.status == 102) {
      console.log("[-] " + ret.status + " : " + ret.message);
      return ret;
    }
    ...
    // await this.page.waitForSelector(".auth_form--error--2I51G").catch((err) => {
    //    status = 102;
    //    message = err.message;
    // });
    // const promptText = this.page.$eval(".auth_form--error--2I51G").textContent;
...
const getTeamMembers = async (username, password) => {
  let figma = new Figma_Integration(username, password);
  //let status, message;
  ret = await figma.launch();
  if (ret.status != 200) {
    return {
      response_code: ret.status,
      message: ret.message,
      selector: ret.selector,
    };
  }

  ret = await figma.login();
  if (ret.status != 200) {
    return {
      response_code: ret.status,
      message: ret.message,
  ...
  let members = await figma.getMembers();
  console.log(members);

  await figma.close();
  return {
    response_code: ret.status,
    message: "Successfully retreived team members",
    members: members,
  };
};

const inviteMember = async (username, password, email) => {
  let figma = new Figma_Integration(username, password);
  await figma.launch();
  await figma.login();
  await figma.inviteMember(email);
  await figma.close();
};

const deleteMember = async (username, password, email) => {
  let figma = new Figma_Integration(username, password);
  await figma.launch();
  await figma.login();
  await figma.deleteMember(email);
  await figma.close();
};

module.exports = { getTeamMembers, inviteMember, deleteMember };
